...
def unpack(data: bytes) -> tuple[MSGTYPE, dict]:
    try:
        type = _type_parser.unpack(data[0])
        attr = {}
        if type == MSGTYPE.RESPONSE:
            status, msg, timestep, flags, map = FORMATTER[MSGTYPE.RESPONSE].unpack(data)
            attr['status'] = status
            attr['msg'] = msg
            ...
            egoidx, scans, poses, vels, iscol, status, elapsed_time = FORMATTER[MSGTYPE.RECV].unpack(data)
            obs = {}
